...
const Footer = () => {
  ...
  return (
    <footer className={`bg-img-general text-white position-relative ${Style.footer}`}>
      <Image src={require("images/footer/background.jpeg")} alt={"footer-background"} fill className={`position-absolute w-100 h-100 ${Style.background}`} />
      <div className={`position-absolute w-100 h-100 ${Style.background}`} />
      <div className="text-center py-4">
        <p className={`fw-bold fs-20px mb-0`}>Follow Us</p>
        <p className={`bold-200 fs-12px mb-1 ${Style.content}`}>Follow for amazing offers, endless inspiration, and unforgettable experiences!</p>
        ...
        <p className={`mb-0 mt-3 fs-12px fw-200`}>&#169; Bookstore, All Rights Reserved</p>
      </div>
    </footer>
  )
}
